...
def insert_data(request):
    ...
    if request.method =='POST' and request.FILES:
        CO = CollegeForm(request.POST,request.FILES)
        if CO.is_valid():
            CO.save()
...
class CollegeData(viewsets.ViewSet):
    def list(self,request):
        ADO=College.objects.all()
        SJD=CollegeMS(ADO,many=True)
        d={'data':SJD.data}
        return render(request,'list.html',d)

    def retrieve(self,request,pk):
        TO=College.objects.get(pk=pk)
        SDO=CollegeMS(TO)
        return Response(SDO.data)

    def update(self,request,pk):
        SPO=College.objects.get(pk=pk)
        SPD=CollegeMS(SPO,data=request.data)
        if SPD.is_valid():
            SPD.save()
    ...
    def partial_update(self,request,pk):
        SPO=College.objects.get(pk=pk)
        SPD=CollegeMS(SPO,data=request.data,partial=True)
        if SPD.is_valid():
            SPD.save()
            return Response({'Updated':'Product is updated'})
        else:
    ...
    def destroy(self,request,pk):
        College.objects.get(pk=pk).delete()
